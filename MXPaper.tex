...
\if0\blind
{
  ...
  \author[1,2]{Joshua T. Vogelstein\thanks{\textit{This work is graciously supported by the Defense Advanced Research Projects Agency (DARPA) SIMPLEX program through SPAWAR contract N66001-15-C-4041, DARPA GRAPHS N66001-14-1-4028, and NIH-TRA 1R01NS092474, "Synaptomes of Mouse and Man".}}}
...
\begin{figure}[t!]
	...
	\end{subfigure}
	\vspace{-1.5\baselineskip}
	\caption{Two-dimensional embedding of samples using PCA followed by coloring using labels given by k-means clustering with \textit{k = 3} using (a) raw data and (b) z-score normalized data. Note that while samples clearly group into 2-3 clusters, the k-means result is visually worse in (b) compared to (a).}
...
\section{Results}
\label{sec:res}
We next used MX to explore a previously unanalyzed dataset. [THIS SECTION MIGHT GET CUT]
